...
if not next(svo.dict) then
  ...
  -- smoke cures
    ...
    slickness = {
      ...
      herb = {
        isadvisable = function()
          return (affs.slickness and not affs.anorexia and not svo.doingaction'slickness') or false -- anorexia is redundant, but just in for now
        end,
  ...
  -- writhes
    ...
    crescendo = {
      ...
      herb = {
        isadvisable = function ()
          return affs.crescendo or false
        end,
  
        oncompleted = function ()
          svo.rmaff("crescendo")
          svo.dict.crescendo.count = 0
          svo.lostbal_herb()
        end,
        ...
        eatcure = {"ash", "stannum"},
        onstart = function ()
          svo.eat(svo.dict.crescendo.herb)
        end,
      ...
      aff = {
        oncompleted = function (number)
          svo.addaffdict(dict.crescendo)
          svo.updateaffcount(svo.dict.crescendo)
        end
      },
      gone = {
        oncompleted = function ()
    ...
    earworm = {
      smoke = {
        isadvisable = function()
          return (affs.earworm and codepaste.smoke_elm_pipe()) or false
        end,
      ...
      aff = {
        oncompleted = function()
          svo.addaffdict(svo.dict.earworm)
        end,
      },
      gone = {
        oncompleted = function() svo.rmaff('earworm') end,
      }
    },
    ...
    fulminated = {
      ...
      aff = {
        oncompleted = function()
          svo.addaffdict(svo.dict.fulminated)
          codepaste.badaeon()
        end,
      },
      gone = {
    ...
    blistered = {
      waitingfor = {
        customwait = 20, -- Austere said it is 20!
      ...
      aff = {
        oncompleted = function()
          svo.addaffdict(svo.dict.blistered)
          if not svo.actions.blistered_waitingfor then svo.doaction(svo.dict.blistered.waitingfor) end
        end
      },
      gone = {
        oncompleted = function()
          svo.rmaff('blistered')
          svo.killaction(svo.dict.blistered.waitingfor)
        end,
      }
    },
  ...
  if svo.haveskillset('discipline') or svo.haveskillset('malignity') or svo.haveskillset('valour') or svo.haveskillset('dominion')  then
    svo.dict.rage = {
      misc = {
        ...
        isadvisable = function()
          if not (conf.rage and bals.rage and (affs.inlove or affs.justice or affs.generosity or affs.pacifism or affs.peace or affs.pyre) and not defc.dragonform and svo.can_usemana()) then return false end
...
if svo.haveskillset('excision') then
  basicdef('fervour', 'perform fervour')
  basicdef('dawnhand', 'perform dawnhand')
  basicdef('faith', 'perform faith')
  basicdef('retributionvow', 'perform retribution', false, 'retribution')
end
