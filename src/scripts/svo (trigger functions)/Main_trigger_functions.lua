...
function svo.defs.got_morph()
  svo.checkaction(svo.dict.waitingformorph.waitingfor)
  if actions.waitingformorph_waitingfor then
    lifevision.add(actions.waitingformorph_waitingfor.p)
  end
end
...
for _, herb in pairs({
  ...
  goldenseal = {'dissonance', 'impatience', 'stupidity', 'dizziness', 'epilepsy', 'shyness', 'depression', 'shadowmadness', 'mycalium', 'sandfever', 'horror', 'fulminated'},
...
for _, smoke in pairs({
  ...
  for _, aff in pairs(smoke) do
    valid['smoke_helped_'..aff] = function()
      local result = svo.checkany(svo.dict[aff].smoke, unpack(other_smoke_cures))
     -- aff twice in the svo.checkany list, first so most cases it gets returned first when it's the only aff
      ...
      svo.smoke_cure = true
      if result.name == aff..'_smoke' then
        lifevision.add(actions[aff..'_smoke'].p, 'cured')
      else
        svo.killaction(svo.dict[result.action_name].smoke)
        svo.checkaction(svo.dict[aff].smoke, true)
        lifevision.add(svo.dict[aff].smoke, 'cured')
      end
    end
    ...
    valid['proper_'..status..name] = function ()
      ...
      if conf.aillusion then
        ...
        if not affs.blackout and (combined:find(limb, 1, true) or combined:find(plural, 1, true)) then
          ...
          if svo.find_until_last_paragraph("^Spinning to the right as \w+ draws %w+ %w+ from its sheath, %w+ delivers a precise slash across your arms%.$", 'pattern') then
...
if svo.haveskillset('discipline') or svo.haveskillset('malignity') or svo.haveskillset('valour') or svo.haveskillset('dominion') then
  ...
  valid.ragecure = function ()
    ...
    if actions.rage_misc then
      ...
      signals.before_prompt_processing:connect(sk.check_emptyrage, 'check for rage not curing anything')
      valid.passive_cure()
    end
  end
else
...
if svo.haveskillset('anathema') then
  function svo.valid.usedanathemabalance()
    svo.checkaction(svo.dict.stolebalance.happened, true)
    lifevision.add(actions.stolebalance_happened.p, nil, 'anathema')
  end
  
  function svo.valid.gotanathemabalance()
    svo.checkaction(svo.dict.gotbalance.happened, true)
    svo.dict.gotbalance.happened.tempmap[#svo.dict.gotbalance.happened.tempmap+1] = 'anathema' -- hack to allow multiple balances at once
    lifevision.add(actions.gotbalance_happened.p)
  end
end

if svo.haveskillset('excision') then
  function svo.valid.usedwrathbalbalance()
    svo.checkaction(svo.dict.stolebalance.happened, true)
    lifevision.add(actions.stolebalance_happened.p, nil, 'anathema')
  end
  
  function svo.valid.gotwrathbalbalance()
    svo.checkaction(svo.dict.gotbalance.happened, true)
    svo.dict.gotbalance.happened.tempmap[#svo.dict.gotbalance.happened.tempmap+1] = 'wrathbal'
    lifevision.add(actions.gotbalance_happened.p)
  end
end
...
if svo.systemloaded then svo.loader.valid_main() end