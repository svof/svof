...
svo.protips = {
  ...
  "You can refill vials directly into rift while using the refiller by setting your container to 'rift', you can try it by using 'vconfig containerid rift'.",
  "Backup your profile and settings regularly so you don't risk losing your things if something bad happens!",
  "The tattoo inker can queue tattoos! You can give it multiple tattoos separate by comma and it will ink them all one after the other.",
  "Got blackouted by a monk and suddenly cannot move anymore? Try to CONCENTRATE.",
  "Svof is 100% open-source which means anyone can contribute to it!",
  "'vshow server' will show you a list of things that serverside curing can do but svo will handle it instead. You can uncheck anything there to make serveride default behavior take control over that specific affliction or defence, or check if you want Svof to handle it instead.",
  "If you have a mindnet artefact or similar, you can enable mindnet support in the settings even if your class doesn't have it. Same goes for lifevision! Try vconfig havemindnet for example (requires a restart upon disabling to remove it from the list)",
  "Svof has a lot of settings that can be configured manually as well as some debug options! Check them out at vconfig list.",
  "Stay tuned! Svof has a clan you can join (CLANHELP SVOF), make sure you are up to date with the news and announcements made there. We also have a discord server -> https://discord.gg/nqmAUr66nY"
}
...
if svo.haveskillset('apostasy') then
  svo.protips[#svo.protips+1] = "The priestreport addon also works for apostates! Try it out."
end
...
if svo.systemloaded then svo.loader.funnies() end