...
-- functions for Svof's aliases, that should call other core functions as necessary to do their work.

svo = svo or {}; svo.loader = svo.loader or {}
svo.modules_version = svo.modules_version or {}
svo.modules_version["svo (alias and defence functions)"] = 1
svo.loader.aliases = function()
...
if svo.haveskillset('discipline') or svo.haveskillset('malignity') or 
  svo.haveskillset('valour') or svo.haveskillset('dominion')  then
  svo.bals.rage = true
end
...
if svo.haveskillset('anathema') then
  svo.bals.anathema = true
end

if svo.haveskillset('excision') then
  svo.bals.wrathbal = true
end
...
end -- end of svo aliases loader

if svo.systemloaded then svo.loader.aliases() end