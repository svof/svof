...
if svo.haveskillset('discipline') then
  installdata.fitness = {
    command = "ab discipline fitness",
    gmcp = {group = 'discipline', name = 'fitness'}}
elseif svo.haveskillset('malignity') then
  installdata.fitness = {
    command = "ab malignity fitness",
    gmcp = {group = 'malignity', name = 'fitness'}}
elseif svo.haveskillset('valour') then
  installdata.fitness = {
    command = "ab valour fitness",
    gmcp = {group = 'valour', name = 'fitness'}}
elseif svo.haveskillset('dominion') then
  installdata.fitness = {
    command = "ab dominion fitness",
    gmcp = {group = 'dominion', name = 'fitness'}}
...
if svo.haveskillset('discipline') then
  installdata.rage = {
    gmcp = {group = 'discipline', name = 'rage'},
  }
end
if svo.haveskillset('malignity') then
  installdata.rage = {
    gmcp = {group = 'malignity', name = 'rage'},
  }
end
if svo.haveskillset('valour') then
  installdata.rage = {
    gmcp = {group = 'valour', name = 'rage'},
  }
end
if svo.haveskillset('dominion') then
  installdata.rage = {
    gmcp = {group = 'dominion', name = 'rage'},
  }
end
  ...
  for name, tbl in svo.config_dict:iter() do
    if conf[name] == nil and tbl.installcheck then
      ...
      if printCmdLine then
        clearCmdLine()
        tempTimer(0, [[appendCmdLine("vconfig ]]..name..[[ ")]])
      end
...
if svo.systemloaded then svo.loader.install() end