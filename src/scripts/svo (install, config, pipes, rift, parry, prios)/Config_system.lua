...
svo.config_dict = svo.pl.OrderedMap {
  ...
  {havemindnet = {
    type = 'boolean',
    onenabled = function () defences.enablelifevision() svo.echof("<0,250,0>Have%s Phylactery of Sixth Sense - added it to defup/keepup.", svo.getDefaultColor()) end,
    ondisabled = function () svo.echof("<250,0,0>Don't%s have Phylactery of Sixth Sense - won't be adding it to defup/keepup.", svo.getDefaultColor()) end,
  }},
  {autoarena = {
    type = 'boolean',
  ...
  {changestype = {
    ...
    check = function (what)
      ...
      return nil, {'full', 'short', 'fullpercent', 'shortpercent'}
    end,
    onset = function ()
  ...
  {commandechotype = {
    ...
    check = function (what)
      ...
      return nil, {'plain', 'fancy', 'fancynewline'}
    end,
    onset = function ()
  ...
  {curemethod = {
    ...
    check = function (what)
      ...
      return nil, {'conconly', 'transonly', 'preferconc', 'prefertrans', 'prefercustom'}
    end,
    onset = function ()
  ...
  {org = {
    ...
    check = function (what)
      ...
      return nil, {'Ashtan', 'Hashan', 'Mhaldor', 'Targossas', 'Cyrene', 'Eleusis', 'None', 'Rogue'}
    end,
    onset = function ()
  ...
  {slowcurecolour = {
    ...
    check = function (what)
      ...
      return nil, "Please use `vlua showColors(searchString)` to see available colors. Default is 'blue'."

    end,
    onset = function ()
      if conf.slowcurecolour == 'off' then conf.slowcurecolour = 'blue' end
  ...
  {autoreject = {
    ...
    check = function (what)
      ...
      return nil, {'black','blacklist', 'white', 'whitelist', 'off', 'on'}
    end,
    onset = function ()
  ...
  {echotype = {
    ...
    check = function (what)
      ...
      local ret = {}
      for k,v in pairs(svo.echos) do table.insert(ret, k) end
      return nil, ret
    end,
    onset = function ()
  ...
  {warningtype = {
    ...
    check = function (what)
      ...
      return nil, {'all', 'prompt', 'none', 'right', 'off'}
    end,
    onset = function ()
  ...
  {burstmode = {
    ...
    check = function (what)
      ...
      local ret = {}
      for k,_ in defdefup do table.insert(ret, k) end
      return nil, ret
    end,
    onshow = function (defaultcolour)
      local tooltip
  ...
  {releasechannel = {
    ...
    check = function (what)
      ...
      return nil, {'stable', 'testing', 'inclient'}
    end,
    onset = function ()
...
if svo.haveskillset('kaido') then
  svo.config_dict:insert(1, 'transmute', {
    ...
    check = function (what)
      ...
      if table.contains({'replaceall', 'replacehealth', 'supplement', 'none'}, what:lower()) then return true end
      return nil, {'anything negative to disable', 'replaceall', 'replacehealth', 'supplement', 'none'}
    end,
    onset = function ()
...
if svo.haveskillset('metamorphosis') then
  ...
  svo.config_dict:insert(1, 'truemorph', {
    type = 'boolean',
    onenabled = function () svo.echof("<0,250,0>Have%s truemorph - expecting instant morphing.", svo.getDefaultColor()) end,
    ondisabled = function () svo.echof("<250,0,0>Don't%s have truemorph - will wait for morphs.", svo.getDefaultColor()) end,
    onshow = "Have truemorph",
    installstart = function () conf.truemorph = nil end,
    installcheck = function () svo.echof("Do you have the Metamorphosis Truemorph skill?") end
  })
...
if svo.haveskillset('discipline') or svo.haveskillset('malignity') or svo.haveskillset('valour') or svo.haveskillset('dominion') or svo.haveskillset('shindo') or svo.haveskillset('kaido') or svo.haveskillset('metamorphosis') then
...
if not conf.releasechannel or true then --force everybody to use the 'inclient' channel at the moment.
  conf.releasechannel = 'inclient'
end
...
function svo.config.set(what, option, echoback)
  ...
  elseif svo.config_dict[what].type == 'string' then
    ...
    if svo.config_dict[what].check and not svo.config_dict[what].check(option) then
      local _, options = svo.config_dict[what].check(option)
      sendf("%s isn't something you can set %s to be.", option, what)
      if type(options) == 'table' then
        sendf("You can set %s to %s", what, svo.concatand(options))
...
if svo.systemloaded then svo.loader.config() end