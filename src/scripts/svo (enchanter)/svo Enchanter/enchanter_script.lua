...
function svo.get_enchant_cost(what, num, exampleItem, costCollection)
...
-- Prints the costs calculated by svo.get_enchant_cost
...
function svo.parse_enchant_arguments(arg)
  ...
  elseif string.starts(arg, "cancel") then
    ...
    if string.starts(arg, "cost") then
      functionToUse = svo.get_enchant_cost
      arg = arg:sub(5)
      getCosts = true
    else
...
function svo.reenchant()
  svo.parse_enchant_arguments(lastOrder)
end
