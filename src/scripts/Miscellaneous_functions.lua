...
function svo.showprompt()
  if conf.singleprompt then svo.bottomprompt:clear() end

  -- https://bugs.launchpad.net/mudlet/+bug/982720 disallows (conf.singleprompt and 'svo.bottomprompt' or 'main')
  -- if conf.paused then
  --   if conf.singleprompt then
  --     decho('svo.bottomprompt', "<255,0,0>(<128,128,128>p<255,0,0>)<0,0,0> ")
  --   else
  ...
  if not conf.customprompt or affs.blackout or svo.innews then
    ...
    if conf.singleprompt then
      ...
      if conf.paused then
        cecho('svo.bottomprompt', "<a_red>(<a_darkgrey>p<a_red>)<black> ")
      end
      appendBuffer('svo.bottomprompt')
    else
      if conf.paused then
    ...
    if conf.singleprompt then
      cecho('svo.bottomprompt', cp.display() or "")
    else
      cecho(cp.display() or "")
    end
  end
...
local yes = {'yes', 'yep', 'yup', 'oui', 'on', 'y', 'da', 'yeah', 'ja'}
local no = {'no', 'nope', 'non', 'off', 'n', 'net', 'nein', 'nah'}
function svo.convert_string(which)
  if type(which) == 'string' then which = which:lower() end
...
if svo.systemloaded then svo.loader.misc() end