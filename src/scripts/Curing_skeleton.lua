...
svo.check_focus = function(sync_mode)
  ...
  if not next(affs) or svo.usingbal('focus') or affs.stun or affs.unconsciousness or not bals.focus
    ...
    or affs.impatience or affs.sandfever or affs.inquisition or (affs.cadmus and not conf.focuswithcadmus) then
      return
  end
...
svo.lostbal_wrathbal = function()
  if not bals.wrathbal then return end
  bals.wrathbal = false
  raiseEvent("svo lost balance", 'wrathbal')
  end
end
...
function sk.dosendprios()
    ...
    if #sk.affqueue > 0 then
        ...
        if sk.queuecount > 4 then
          tempTimer(1+getNetworkLatency(),function()
